...
public class CashToAccountChannelPromoProcessor extends BaseChannelProcessor {
	...
	public Result CashInRemit(CashInRemitHelper helper) {
		...
		try {
			...
			trx.voucherCode = helper.voucherCode;
			...
			if (helper.voucherCode != null && !helper.voucherCode.equals("")) {
				Voucher voucher = Utility.checkVoucher(helper);
				if (voucher != null) {
	...
	public Result CashOutInquiry(CashOutInquiryHelper helper) {
		Logger.info("-------------------------------------------------------------");
		Logger.info(":: PROCESSOR CASH TO ACCOUNT CHANNEL PROMO CASHOUT INQUIRY :: ");
		Logger.info("=============================================================");
		ObjectNode result = Json.newObject();
		try {
	...
	public Result CashOutValidate(CashOutValidateHelper helper) {
		Logger.info("--------------------------------------------------------------");
		Logger.info(":: PROCESSOR CASH TO ACCOUNT CHANNEL PROMO CASHOUT VALIDATE :: ");
		Logger.info("==============================================================");
		ObjectNode result = Json.newObject();
		try {
	...
	public Result CashOutCollect(CashOutCollectHelper helper) {
		Logger.info("-------------------------------------------------------------");
		Logger.info(":: PROCESSOR CASH TO ACCOUNT CHANNEL PROMO CASHOUT COLLECT :: ");
		Logger.info("=============================================================");
		ObjectNode result = Json.newObject();
