...
public class BankDepositProcessor extends BaseChannelProcessor {
	...
	public Result CashInInquiry(CashInInquiryHelper helper) {
		...
		try {
			...
			helper.beneficiaryAmount = helper.beneficiaryAmount.setScale(2, RoundingMode.FLOOR);
			...
			if (helper.beneficiaryAccount != null) {
				...
				TransferResponse responseInquiry = Utility.fundInquiryBankDeposit(helper,additionalFee);
				Logger.debug("RESPONSE : "+Json.toJson(responseInquiry));
				if (responseInquiry.status == 0) {
					transactionInquiry.validationId = responseInquiry.accessToken;
					transactionInquiry.accountName = responseInquiry.receiverName;
					transactionInquiry.status = TransactionInquiry.TransactionInquiryStatus.SUCCESS.getCode();
					transactionInquiry.state = TransactionInquiry.TransactionInquiryState.DONE.getCode();
					transactionInquiry.inquiryResponseCode = responseInquiry.referenceStatus != null ? responseInquiry.referenceStatus+"" : responseInquiry.referenceStatus;
					transactionInquiry.inquiryResponseMessage = responseInquiry.referenceMessage != null ? responseInquiry.referenceMessage : responseInquiry.referenceStatus;
