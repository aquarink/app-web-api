...
public class BankDepositProcessor extends BaseChannelProcessor {
	...
	public Result CashInInquiry(TransactionInquiry inquiry) {
		...
		try {
			...
			Logger.debug("DATA : "+inquiry+" :: "+forexReference.rate);
			...
			Channel channel = Channel.findByCode("07");
			...
			if (responseInquiry.status == 0) {
				inquiry.validationId = responseInquiry.accessToken;
				inquiry.accountName = responseInquiry.receiverName;
				...
				inquiry.state = TransactionInquiry.TransactionInquiryState.DONE.getCode();
				inquiry.inquiryResponseCode = responseInquiry.referenceStatus != null ? responseInquiry.referenceStatus+"" : responseInquiry.referenceStatus;
				inquiry.inquiryResponseMessage = responseInquiry.referenceMessage != null ? responseInquiry.referenceMessage : responseInquiry.referenceStatus;
				inquiry.update();
				result.put("status", "1");
